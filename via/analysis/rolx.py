...
def extract_rolx_roles(G, roles=2):
    ...
    # basis, coef = get_optimal_factorization(V)
...
def recursive_feature_array(G, func, n):
    ...
    if nx.get_node_attributes(G, attr_name):
        result = sorted(nx.get_node_attributes(G, attr_name).items())
        result = np.array([v for node, v in result])
        return result
    ...
    if n == 0:
        ...
        result = result * 1.0
        nx.set_node_attributes(G, {i: v for i, v in enumerate(result)}, name=attr_name)
        return result
    ...
    for v in sorted(G.nodes):
        ...
        v_stats = np.concatenate(
            (neighbor_sums_vec, neighbor_avgs_vec), axis=0)
        all_neighbor_stats.append(v_stats)

    nx.set_node_attributes(
        G, {i: v for i, v in enumerate(all_neighbor_stats)}, name=attr_name)
...
def degree(G):
    ...
    degrees = sorted(G.degree(weight='weight'))
    return [degree for node, degree in degrees]


def vertex_egonet(G, v):
    """ Computes the number of edges in the ego network of the vertex v. """
    neighbors = G.neighbors(v)
    neighbors = [nbr for nbr in neighbors] + [v]
    ego_network = G.subgraph(neighbors)
    ego_edges = len(ego_network.edges)
    return ego_edges


def egonet(G):
    """ Computes the ego network for all vertices v in G. """
    return [vertex_egonet(G, v) for v in sorted(G.nodes)]
...
def vertex_egonet_out(G, v):
    ...
    neighbors = [nbr for nbr in neighbors] + [v]
    ego_network = G.subgraph(neighbors)
    ego_edges = ego_network.size(weight='weight')
    degree_sum = sum([G.degree(v, weight='weight') for v in neighbors])
    # Summing over degree will doublecount every edge within the ego network
    out_edges = degree_sum - 2*ego_edges
    return out_edges


def egonet_out(G):
    """ Computes the number of outgoing ego network edges for every vertex in G. """
    return [vertex_egonet_out(G, v) for v in sorted(G.nodes)]
...
def get_optimal_factorization(V, min_roles=2, max_roles=6, min_bits=1, max_bits=10):
    ...
    max_roles = min(
        max_roles, V.shape[1])  # Can't have more possible roles than features
    ...
    for i in range(num_role_options):
        ...
        for j in range(num_bit_options):
            bits = min_bits + j
            enc_W, enc_H, enc_cost, err_cost = description_length(
                V, fctr_res, bits)
   ...
   # print mat_total_cost
    print('min cost @', i, ' or at ', min_roles)
    ...
    print("%s, %s, (%s, %s, %s)" % (min_role_value, min_bit_value,
                                    min_std_enc_cost, min_std_err_cost, min_total_std_cost))
...
def make_sense(G, H):
    ...
    G_copy = G.copy()
    # add distance (probabilistic case, we flip the sign and increment by 1 to preserve nonnegativity)
    nx.set_edge_attributes(G_copy, {edge: -1 * G[edge[0]][edge[1]]['weight'] + 1 for edge in G.edges}, name='distance')
    feature_matrix = []
    # in-degree of a node v
    feature_matrix.append([x[1] for x in sorted(
        dict(G_copy.in_degree(weight='weight')).items())])
    # in-degree of a node v
    feature_matrix.append([x[1] for x in sorted(
        dict(G_copy.out_degree(weight='weight')).items())])
    # sum of the fraction of all-pairs shortest paths passing through v
    feature_matrix.append([x[1] for x in sorted(
        dict(nx.betweenness_centrality(G_copy)).items())])
    # closeness to all other nodes via outward edges of v
    feature_matrix.append([x[1] for x in sorted(
        dict(nx.closeness_centrality(G_copy, distance='distance')).items())])
    # closeness to all other nodes via inward edges of v
    feature_matrix.append([x[1] for x in sorted(
        dict(nx.closeness_centrality(G_copy.reverse(), distance='distance')).items())])
    # influence based on in-degree
    feature_matrix.append([x[1] for x in sorted(
        dict(nx.pagerank(G_copy, weight='weight')).items())])
...
def complete_factor(H, M, h_on_left=True):
    ...
    result = minimize(residual, guess, args=(
        H, M), method='L-BFGS-B', bounds=bounds)
